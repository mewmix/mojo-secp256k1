...
from secp256k1.fe import (
    ...
    _fe_from_int, _fe_to_int, fe_is_zero, fe_equal
)
...
fn test_field_basic() raises:
    ...
    # Test zero and one
    var zero = fe_zero()
    assert_true(fe_is_zero(zero), "fe_zero should be 0")
    
    var one = fe_one()
    assert_true(fe_equal(one, _fe_from_int(1)), "fe_one should be 1")
    
    # Test addition
    var a = _fe_from_int(42)
    var b = _fe_from_int(58)
    var sum = fe_add(a, b)
    assert_true(fe_equal(sum, _fe_from_int(100)), "42 + 58 should be 100")
    
    # Test subtraction
    var diff = fe_sub(b, a)
    assert_true(fe_equal(diff, _fe_from_int(16)), "58 - 42 should be 16")
    
    # Test negation
    var neg_a = fe_neg(a)
    var check_zero = fe_add(a, neg_a)
    assert_true(fe_is_zero(check_zero), "a + (-a) should be 0")
    
    # Test multiplication
    var prod = fe_mul(a, b)
    assert_true(fe_equal(prod, _fe_from_int(2436)), "42 * 58 should be 2436")
    
    # Test squaring
    var sqr = fe_sqr(a)
    assert_true(fe_equal(sqr, _fe_from_int(1764)), "42^2 should be 1764")
...
fn test_field_modular() raises:
    ...
    assert_true(fe_equal(prod, fe_one()), "x * x^-1 should be 1")
